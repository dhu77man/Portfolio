var blackhole = function(parent){
    ...
    function init(image) {
        ...
        canvas.width  = parent.offsetWidth >= parent.offsetHeight ? parent.offsetWidth : parent.offsetHeight;
        canvas.height = parent.offsetWidth >= parent.offsetHeight ? parent.offsetWidth : parent.offsetHeight;

        parent.appendChild(canvas);

        mouse = {x: canvas.width/9, y: canvas.height - 80, moved: false};
    ...
    function render() {
        ...
        gl.uniform1f(locationOfMass, curblackholeMass*0.00001);
        gl.uniform2f(locationOfMouse, mouse.x, mouse.y - app.scrollTop());
    ...
    window.addEventListener('load', function(event){
        var image = new Image();
        image.src = 'bin/images/israel.jpg';
        image.onload = function() {
            var c = document.createElement('canvas'),
            ctx = c.getContext('2d');

            c.width  = parent.offsetWidth >= parent.offsetHeight ? parent.offsetWidth : parent.offsetHeight;
            c.height = parent.offsetWidth >= parent.offsetHeight ? parent.offsetWidth : parent.offsetHeight;

            ctx.translate(0, c.height);
            ctx.rotate(Math.PI);
            ctx.scale(-1, 1);
            drawImageProp(ctx, image, 0, -c.height*0.28, c.width, c.height);

            init(ctx.getImageData(0, 0, c.width, c.height));
        }
